...
class SCCLFeatureExtractor(nn.Module):
    def __init__(self, bert_model, tokenizer, cluster_centers=None, alpha=1.0):
        super(SCCLFeatureExtractor, self).__init__()
    ...
    def forward(self, input_ids, attention_mask, task_type="virtual"):
        """
        get the embedding of bert, return text embedding
        Args:
            inputs_ids: [B, N,]
        Return:
            mean_output: [B, N, D]
        ...
        if task_type == "evaluate":
            ...
            mean_output_1 = self.batch_mean_embeddings(input_ids_1, attention_mask_1)
            mean_output_2 = self.batch_mean_embeddings(input_ids_2, attention_mask_2)
            ...
            attention_mask_1, attention_mask_2, attention_mask_3 = torch.unbind(
                attention_mask, dim=1
            )

            mean_output_1 = self.batch_mean_embeddings(input_ids_1, attention_mask_1)
            mean_output_2 = self.batch_mean_embeddings(input_ids_2, attention_mask_2)
            mean_output_3 = self.batch_mean_embeddings(input_ids_3, attention_mask_3)
            return mean_output_1, mean_output_2, mean_output_3

        else:
            raise Exception(
                "Bert encoding type error! Options: [evaluate, virtual, explicit]"
            )

    def batch_mean_embeddings(self, input_ids, attention_mask):
        """
        use mean embedding for contrastive learning
        """
    ...
    def get_cluster_prob(self, embeddings):
        """
        get the probability of cluster
        """
    ...
    def local_consistency(self, embd0, embd1, embd2, criterion):
        """
        get the cos similarity of embeddings
        """
