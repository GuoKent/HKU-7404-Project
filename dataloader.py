...
class ExplitAugSamples(Dataset):
    ...
    def __getitem__(self, idx):
        return {
            ...
            "augmentation_1": self.train_x1[idx],
            "augmentation_2": self.train_x2[idx],
...
def explict_augmentation_loader(args):
    ...
    # train_data = pd.read_table(os.path.join(args.datapath, args.dataname+".txt"), sep='\t')
    ...
    # print(f"dataloader {type(train_label)},{np.unique(train_label)}")
